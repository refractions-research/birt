...
public class FontConfigReaderTest extends TestCase
{
	...
	public void testFontMappingHandler( ) throws IOException,
			FactoryConfigurationError, ParserConfigurationException,
			SAXException
	{
		fontMappingManager = getFontMappingManager( "fontsConfig3.xml",
				Locale.CHINA );
		...
		CompositeFont cf = fontMappingManager
			...
			SAXException
	{
		...
		CompositeFont overriddenFont = fontMappingManager
				.getCompositeFont( "font overridden" );
		assertEquals( "font in composite-font", overriddenFont
				.getUsedFont( (char) 0 ) );
		assertEquals( "font in composite-font", overriddenFont
				.getUsedFont( (char) 0x7F ) );

		CompositeFont notOverriddenFont = fontMappingManager
				.getCompositeFont( "font not overridden" );
		assertEquals( "font in all-fonts", notOverriddenFont
				.getUsedFont( (char) 0 ) );
		assertEquals( "font in all-fonts", notOverriddenFont
				.getUsedFont( (char) 0x7F ) );
		CompositeFont allFonts = fontMappingManager
				.getCompositeFont( "all-fonts" );
			...
			protected URL getConfigURL( String configName )
			{
				URL fileURL = getClass( ).getResource(
						"/org/eclipse/birt/report/engine/layout/pdf/font/" +
								testDir + "/" + configName + ".xml" );
				return fileURL;
			}
		};

		FontMappingManager manager = factory.getFontMappingManager( format,
				Locale.CHINESE );
	...
	private FontMappingManager getFontMappingManager( String configFile,
			Locale locale ) throws IOException, FactoryConfigurationError,
			ParserConfigurationException, SAXException
	{
		...
		System.out.println( "load font config in " + url + " cost " +
				( end - start ) + "ms" );

		return FontMappingManagerFactory.getInstance( )
				.createFontMappingManager( config, locale );
	}

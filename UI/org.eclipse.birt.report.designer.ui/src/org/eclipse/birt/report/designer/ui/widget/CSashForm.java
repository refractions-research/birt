...
public class CSashForm extends Composite
{
	...
	{
		// checkWidget();
		return ( sashStyle & SWT.VERTICAL ) != 0 ? SWT.HORIZONTAL
				: SWT.VERTICAL;
	}
	...
	void onDragSash( Event event )
	{
		...
		if ( getOrientation( ) == SWT.HORIZONTAL )
		{
			...
			if ( data1 == null || !( data1 instanceof CSashFormData ) )
			{
				...
				( (CSashFormData) data1 ).weight = ( ( (long) b1.width << 16 )
						+ area.width - 1 )
						/ area.width;
			...
			// in the case (data1 instanceof CSashFormData) is true
			{
				...
				if ( cData.exactWidth == CSashFormData.NOT_SET )
				{
					cData.weight = ( ( (long) b1.width << 16 ) + area.width - 1 )
							/ area.width;
				}
				else
			...
			if ( data2 == null || !( data2 instanceof CSashFormData ) )
			{
				...
				( (CSashFormData) data2 ).weight = ( ( (long) b2.width << 16 )
						+ area.width - 1 )
						/ area.width;
			...
			// in the case (data1 instanceof CSashFormData) is true
			{
				...
				if ( cData.exactWidth == CSashFormData.NOT_SET )
				{
					cData.weight = ( ( (long) b2.width << 16 ) + area.width - 1 )
							/ area.width;
				}
				else
				// calculate the exact width
		...
		else
		{
			...
			if ( data1 == null || !( data1 instanceof CSashFormData ) )
			{
				data1 = new CSashFormData( );
				c1.setLayoutData( data1 );
				( (CSashFormData) data1 ).weight = ( ( (long) b1.height << 16 )
						+ area.height - 1 )
						/ area.height;
			}
			else
			// in the case (data1 instanceof CSashFormData) is true
			{
				...
				if ( cData.exactWidth == CSashFormData.NOT_SET )
				{
					cData.weight = ( ( (long) b1.height << 16 ) + area.height - 1 )
							/ area.height;
				}
				else
				// calculate the exact width
			...
			if ( data2 == null || !( data2 instanceof CSashFormData ) )
			{
				data2 = new CSashFormData( );
				c2.setLayoutData( data2 );
				( (CSashFormData) data2 ).weight = ( ( (long) b2.height << 16 )
						+ area.height - 1 )
						/ area.height;
			...
			}else// in the case (data1 instanceof CSashFormData) is true
			{
				...
				if ( cData.exactWidth == CSashFormData.NOT_SET )
				{
					cData.weight = ( ( (long) b2.height << 16 ) + area.height - 1 )
							/ area.height;
				}
				else
				// calculate the exact width
	 ...
	 * 
	 * @param sizes
	 *            the absolute width of each child
	 * 
	 * @exception SWTException
	 *                <ul>
	 *                <li>ERROR_WIDGET_DISPOSED - if the receiver has been
...
//				SWT.error( SWT.ERROR_INVALID_ARGUMENT );
		...
		for ( int i = 0; i < cArray.length; i++ )
		{
			...
			if ( data == null || !( data instanceof CSashFormData ) )
			{
				...
				if(weights[i] != CSashFormData.NOT_SET)
				{
					( (CSashFormData) data ).weight = ( ( (long) weights[i] << 16 )
							+ total - 1 )
							/ total;
				}else
				{
			...
			// the case (data instanceof CSashFormData) is true
			{
				...
				if ( weights[i] != CSashFormData.NOT_SET )
				{
					cData.weight = ( ( (long) weights[i] << 16 )
							+ total - 1 )
							/ total;
				}else
				{

...
public class CSashFormLayout extends Layout
{
			...
			boolean flushCache )
	{
		...
		for ( int i = 0; i < cArray.length; i++ )
		{
			if ( vertical )
			{
				Point size = cArray[i].computeSize( wHint,
						SWT.DEFAULT,
						flushCache );
				if ( size.y > maxValue )
				{
			...
			{
				Point size = cArray[i].computeSize( SWT.DEFAULT,
						hHint,
						flushCache );
				if ( size.x > maxValue )
				{
		...
		if ( ratios[maxIndex] > 0 )
		{
			int sashwidth = sashForm.sashes.length > 0 ? sashForm.SASH_WIDTH
					...
					* 2 : sashForm.SASH_WIDTH;
			if ( vertical )
			{
				height += (int) ( total * maxValue / ratios[maxIndex] )
						+ ( cArray.length - 1 )
						* sashwidth + obligatedWidth;
			}
			else
			{
	...
	protected void layout( Composite composite, boolean flushCache )
	{
		...
		{			
			...
			{
				width = (int) ( ratios[0]
								* ( remainWidth - sashes.length * sashwidth ) / total );
			}else
			{
		...
		else
		{
			...
			{
				height = (int) ( ratios[0]
								* ( remainHeight - sashes.length * sashwidth ) / total );
			}else
			{
				CSashFormData cData = (CSashFormData) controls[0].getLayoutData( );
			...
			if ( controls.length > 1 )
			{
				sashes[sashes.length - 1].setBounds( area.x,
						y,
						area.width,
						sashwidth );
				y += sashwidth;

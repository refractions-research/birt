...
public class TableCellKeyDelegate extends GraphicalViewerKeyHandler
{
	...
	/**
	 * @return <code>true</code> if the keys pressed indicate to traverse
	 *         inside a container
	 */
	...
	/**
	 * @return <code>true</code> if the keys pressed indicate to traverse to
	 *         the parent of the currently focused EditPart
	 */
	...
	boolean navigateNextSibling( KeyEvent event, int direction, List list )
	{
		...
		if ( epStart instanceof AbstractCellEditPart )
		{
			if ( ( event.stateMask & SWT.SHIFT ) != 0 )
			{
				 ...
				 */
				List parts = findTableCellSiblings( list,
						pStart,
						direction,
						epStart );
			...
			else
			{
				 ...
				 */
				EditPart next = findTableCellSibling( list,
						pStart,
						direction,
						epStart );
	...
	void translateRectangle( AbstractTableEditPart table, Rectangle rect, int direction )
	{
		...
		/**
		 * Checks if there has one Cell across all the columns or rows in the
		 * given rectangle.
		 */
		...
		/**
		 * Checks if there has one vertical line which divides the rectangle
		...
		/**
		 * Checks if there has one horizontal line which divides the rectangle
		 * into two complete parts.
		 */
		if ( VMovable )
		{
		...
		/**
		 * If vertically movable and last vertical movement direction is
		 * negative, or not vertically movable and current movement direction is
		 * North, translate the coordinate.
		 */
		if ( ( lastVDir < 0 && VMovable )
				|| ( !VMovable && direction == PositionConstants.NORTH ) )
		{
		...
		/**
		 * If horizontally movable and last horizontal movement direction is
		 * negative, or not horizontally movable and current movement direction
		 * is West, translate the coordinate.
		 */
		if ( ( lastHDir < 0 && HMovable )
				|| ( !HMovable && direction == PositionConstants.WEST ) )
		{
		...
		switch ( direction )
		{
			case PositionConstants.NORTH :
				...
				if ( rect.height <= 0 )
				{
					 ...
					 */
					rect.height -= 1;
					boolean refined = TableCellSelectionHelper.increaseSelectionRectangle( rect,
							table );

					while ( refined )
					{
						refined = TableCellSelectionHelper.increaseSelectionRectangle( rect,
								table );
					}
				}
				else
				{
					 ...
					 */
					rect.height -= 1;
					boolean refined = TableCellSelectionHelper.decreaseSelectionRectangle( rect,
							table,
							direction );

					while ( refined )
					{
						refined = TableCellSelectionHelper.decreaseSelectionRectangle( rect,
								table,
								direction );
					}
				}

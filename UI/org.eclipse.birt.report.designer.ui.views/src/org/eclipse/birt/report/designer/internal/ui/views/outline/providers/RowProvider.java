...
public class RowProvider extends DefaultNodeProvider
{
			...
			IMenuManager menu )
	{
		menu.add( new InsertAction( object,
				...
				InsertAction.ABOVE ) );
		menu.add( new InsertAction( object,
				Messages.getString( "RowProvider.action.text.below" ), //$NON-NLS-1$
				ReportDesignConstants.ROW_ELEMENT,
				InsertAction.BELOW ) );
	...
	protected boolean performInsert( Object model, SlotHandle slotHandle,
			String type, String position, Map extendedData ) throws Exception
	{
	 ...
	 * (non-Javadoc)
	 * 
	 * @see org.eclipse.birt.report.designer.internal.ui.views.DefaultNodeProvider#performDelete(org.eclipse.birt.model.api.DesignElementHandle)
	 */
			...
			throws SemanticException
	{
		int rowNumber = HandleAdapterFactory.getInstance( )
				...
				.getRowNumber( );
		TableHandleAdapter adapter = HandleAdapterFactory.getInstance( )
				.getTableHandleAdapter( getRoot( handle ) );
